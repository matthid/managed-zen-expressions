...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| simple-few    | **148 ns** | 457 ns (3.1×) | 3 571 ns (24×) | 806 ns | 926 ns | 4 328 ns |
| complex-few   | **306 ns** | 558 ns (1.8×) | 6 436 ns (21×) | 1 242 ns | 1 280 ns | 6 559 ns |
| simple-many   | **2 340 ns** | 2 478 ns (1.1×) | 59 053 ns (25×) | 9 200 ns | 10 750 ns | 71 958 ns |
| complex-many  | **2 163 ns** | 2 183 ns (1.0×) | 67 934 ns (31×) | 5 282 ns | 5 360 ns | 71 742 ns |
| alloc-string  | **306 ns** | 969 ns (3.2×) | 4 356 ns (14×) | 777 ns | 1 462 ns | 5 460 ns |
| alloc-object  | **548 ns** | 1 871 ns (3.4×) | 9 804 ns (18×) | 1 156 ns | 2 291 ns | 27 439 ns |
| alloc-array   | **1 943 ns** | 5 227 ns (2.7×) | 25 382 ns (13×) | 3 712 ns | 6 123 ns | 18 905 ns |
...
| --- | ---: | ---: | ---: | ---: |
| heavy-sum-1k (sum of 1 000, scalar result) | 3.5 µs | **2.95 µs** ✅ | 60.5 µs | **26.6 µs** ✅ |
| heavy-arith-200 (200-term arithmetic, scalar) | **9.6 µs** | 9.5 µs | **38.1 µs** | 57.8 µs |
| heavy-filter-1k (~500-element result) | **59.5 µs** | 108.8 µs | **116.2 µs** | 131.2 µs |
| heavy-map-1k (1 000-element result) | **79.2 µs** | 194.8 µs | **148.3 µs** | 222.0 µs |
| heavy-map-objects-100 (100 objects result) | **33.5 µs** | 94.7 µs | **63.8 µs** | 134.5 µs |
...
- **Native wins only on `heavy-sum-1k`** — heavy *compute* with a **scalar**
  ...
  (pure: ~16% faster), and on the JSON path native is still **~2.3× faster** even
...
- **JSON reader optimized:** `ZenJson` was rewritten on `Utf8JsonReader` (single
  pass, no `JsonDocument` double-number-parse) with `ArrayPool`-backed array growth.
  This sped up **every** JSON path — standard scenarios ~12–25%, and `heavy-sum-1k`
  JSON **84 → 61 µs** — but native's `serde` still parses the 1 000-element array
  faster (≈27 µs). That remaining gap is `serde`'s maturity on bulk number
